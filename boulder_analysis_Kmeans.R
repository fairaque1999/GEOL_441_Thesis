...
for(site in sites){
  datfn<-paste("/Users/fairuzishraque/Documents/Mars_Glacier_Boulders/Detection_Data/Centerlines/site_",site,".csv",sep="")
  ...
  ##  Choose best k
  #####################################################
  x<-dat$d_down  ###take x values
  k<- 1:25  ###cluster values to check
  result<-Ckmeans.1d.dp(x,k=k) ###fit all cluster models
  
  outputfn<-paste("/Users/fairuzishraque/Documents/Mars_Glacier_Boulders/Detection_Data/Centerlines/site_",site,"_BICPLOT.pdf",sep="")
  pdf(file = outputfn)
  ...
  outputfn<-paste("/Users/fairuzishraque/Documents/Mars_Glacier_Boulders/Detection_Data/Centerlines/site_",site,"_OPTCLUSTERPLOT.pdf",sep="")
  pdf(file = outputfn)
  ggdat<-data.frame(x=dat$d_down,
                    y=dat$d_center,
                    cluster=factor(result$cluster))
